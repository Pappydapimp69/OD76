...
function bankRunB99(dead){
 ...
 S.b99Banked=true;
 const collected=Math.max(0,Math.floor(S.runHearts||0)),earned=dead?Math.floor(collected/2):collected;
 ...
 ranchB99.report=dead
   ?`Battle test ${ranchB99.tests}: fell on stage ${S.stage}. Pip brought home ♥ ${earned} of ${collected} and came back worn out.`
   :`Battle test ${ranchB99.tests}: came home after clearing stage ${S.stage}. Pip brought home all ♥ ${earned}.`;
 saveRanchB99();
 return earned;
}
...
function openRanchGateB99(){
 ...
 const n=Math.max(0,Math.floor(S.runHearts||0));
 $("ranchGateTextB99").textContent=`Pip has collected ♥ ${n} this test. Go home now and the ranch banks all of it. Fall in battle and it banks half.`;
