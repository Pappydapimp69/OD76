...
if (files.length !== 114 || files.at(-1) !== 'b21-114.js' || files[98] !== 'b21-99.js') throw Error('B117 module order');
...
new vm.Script(fs.readFileSync(path.join(__dirname, 'b117-guardian-checks.js'), 'utf8')).runInContext(dom.getInternalVMContext());
results.push(...vm.runInContext('runGuardianChecksB117()', dom.getInternalVMContext()));
