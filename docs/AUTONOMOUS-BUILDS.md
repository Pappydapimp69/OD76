...
Direct build after B98: B99 Pip Ranch in `b21-88.js`. User-requested; does not increment the loop count.

Direct build after B99: B100 Ranch world in `b21-89.js`. User-requested; does not increment the loop count.

Direct build after B100: B101 Unspent hearts, a fix in `b21-88.js`. User-requested; does not increment the loop count.

Direct build after B101: B102 Heart Refinery in `b21-90.js`. User-requested; does not increment the loop count.
...
Direct build after B105: B106 Overgrowth in `b21-94.js`. User-requested; does not increment the loop count.

Direct build after B106: B107 Gate refinery timer in `b21-95.js`. User-requested; does not increment the loop count.

Direct build after B107: B108 Stage curve and ranks in `b21-96.js`. User-requested; does not increment the loop count.

Direct build after B108: B109 Pip actions in `b21-97.js`. User-requested; does not increment the loop count.

Direct build after B109: B110 Bag on Select in `b21-98.js`. User-requested; does not increment the loop count.

Direct build after B110: B111 Start at the ranch in `b21-99.js`. User-requested; does not increment the loop count.
...
Direct build after B113: B114 Rested for battle in `b21-102.js`. User-requested; does not increment the loop count.

Next: select loop 16 / B115 from the B114 baseline. No gameplay implementation is currently unfinished.
...
Continuation automation: `od76-finish-50-zanegpt-build-loops` (every 30 minutes on this task; pause at completion).
